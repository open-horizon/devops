...
- You must pass in the same variable customizations that you did to deploy-mgmt-hub.sh, and the passwords and tokens that were automatically generated by deploy-mgmt-hub.sh. The variables can be passed to deploy-mgmt-hub.sh either by exporting them in the environment, or by setting them in a config file and passing it to this script using the -c flag.
